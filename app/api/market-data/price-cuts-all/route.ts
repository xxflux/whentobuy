...
export async function GET() {
  try {
    // Fetch Price Cuts data for all regions
    const regionDataPromises = REGIONS.map(async (region) => {
      ...
      if (error) {
        console.error(`Error fetching Price Cuts for ${region}:`, error);
        return { region, data: [] };
      }
    ...
    console.error('Error fetching all regions Price Cuts:', error);
